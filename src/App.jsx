...
function SC({num,label,color,title,children}){
  return (
    <div style={{background:C.white,borderRadius:18,padding:"2rem",marginBottom:"1.25rem",borderLeft:`4px solid ${color}`,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:"1.25rem",flexWrap:"wrap"}}>
        ...
        <h2 style={{fontSize:20,fontWeight:600,fontFamily:serif,color:C.ink,margin:0,lineHeight:1.3}}>{title}</h2>
      </div>
      <div style={{fontSize:16,lineHeight:1.78,color:C.ink,fontFamily:sans}}>{children}</div>
    </div>
  );
}
...
function Cd({lang,code}){
  return (
    <div style={{background:C.codeBg,borderRadius:12,padding:"1.25rem 1.5rem",margin:"1rem 0",overflowX:"auto"}}>
      ...
      <pre style={{margin:0,fontFamily:mono,fontSize:13,lineHeight:1.75,color:C.codeText,whiteSpace:"pre"}}>{code}</pre>
    </div>
  );
}
function Tb({h,r,ac}){
...
function Xtra({x,ac}){
  ...
  if(x.t==="c") return <Cd lang={x.lang} code={x.code}/>;
...
// ── quiz engine ───────────────────────────────────────────────────────────────
function Quiz({qs,ac,onComplete,isCompleted}){
  ...
  return (
    <div style={{background:C.white,borderRadius:20,padding:"2rem",marginBottom:"1.5rem",border:`1.5px solid ${ac||C.terra}30`}}>
      ...
      <h2 style={{fontSize:24,fontWeight:700,fontFamily:serif,color:C.ink,margin:"0.75rem 0 0.4rem"}}>Aplique o que você leu</h2>
      <p style={{fontSize:15,color:C.muted,fontFamily:sans,margin:"0 0 1.5rem"}}>{qs.length} perguntas sobre os conceitos do módulo.</p>
      {qs.map((q,i)=> (
        <div key={i} style={{background:C.cream,borderRadius:16,padding:"1.5rem",marginBottom:"1rem",border:`1px solid ${C.border}`}}>
...
// ── checklist engine ──────────────────────────────────────────────────────────
function Chk({title,sub,steps,ac,msg,onComplete,onNext,isCompleted}){
  ...
  return (
    <div style={{background:C.white,borderRadius:20,padding:"2rem",marginBottom:"1.5rem",border:`1.5px solid ${ac||C.terra}30`}}>
      ...
      <h2 style={{fontSize:24,fontWeight:700,fontFamily:serif,color:C.ink,margin:"0.75rem 0 0.375rem"}}>{title}</h2>
      <p style={{fontSize:15,color:C.muted,fontFamily:sans,margin:"0 0 1.25rem"}}>{sub}</p>
...
function MV({mod,onComplete,onNext,isCompleted}){
  const [os,setOs]=useState("windows");
  return (
    <div style={{background:C.cream,padding:"2rem 1rem"}}>
      <div style={{maxWidth:720,margin:"0 auto"}}>
        ...
        <div style={{marginBottom:"2.75rem"}}>
          ...
          <h1 style={{fontSize:"clamp(32px,6vw,48px)",fontWeight:700,lineHeight:1.1,fontFamily:serif,color:C.ink,margin:"0 0 0.75rem"}}>
            {mod.t1}<br/><span style={{color:mod.color}}>{mod.t2}</span>
          </h1>
          <p style={{fontSize:17,lineHeight:1.65,color:C.muted,fontFamily:sans,maxWidth:540,margin:"0 0 1.25rem"}}>{mod.subtitle}</p>
        ...
        {/* sections */}
        {mod.sections.map(s=> (
          <SC key={s.num} num={s.num} label={s.label} color={s.sc} title={s.title}>
            <p style={{margin:0,lineHeight:1.78}}>{s.body}</p>
            {(s.xs||[s.x]).filter(Boolean).map((x,i)=> <Xtra key={i} x={x} ac={mod.color}/>)}
          </SC>
        ))}
...
export default function App(){
  ...
  const mark=id=>{if(!done(id))setCompleted(p=>[...p,id]);};
  ...
  const goTo=(id,i)=>{if(unlocked(i)){setActiveId(id);window.scrollTo({top:0,behavior:"smooth"});}};
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:C.cream,fontFamily:sans}}>
      {/* sidebar */}
      <aside style={{width:260,minWidth:260,background:C.white,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100vh",overflowY:"auto",flexShrink:0}}>
        <div style={{padding:"1.375rem 1.25rem 1rem",borderBottom:`1px solid ${C.border}`}}>
        ...
        <nav style={{padding:"0.5rem 0",flex:1}}>
          {MODS.map((m,i)=> (
            <button key={m.id} onClick={()=>goTo(m.id,i)} disabled={!unlocked(i)} style={{width:"100%",textAlign:"left",padding:"0.6rem 1.25rem 0.6rem 1rem",background:m.id===activeId?C.terraLight:"transparent",border:"none",borderLeft:`3px solid ${m.id===activeId?C.terra:"transparent"}`,cursor:unlocked(i)?"pointer":"not-allowed",opacity:unlocked(i)?1:0.4,display:"flex",alignItems:"center",gap:10}}>
      ...
      <main style={{flex:1,minWidth:0,display:"flex",flexDirection:"column"}}>
        <header style={{display:"flex",alignItems:"center",gap:12,padding:"0.875rem 1.25rem",background:C.white,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:30}}>
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={goPrev} disabled={!hasPrev} style={{width:32,height:32,borderRadius:8,border:`1px solid ${C.border}`,background:C.white,cursor:hasPrev?"pointer":"not-allowed",opacity:hasPrev?1:0.35,display:"flex",alignItems:"center",justifyContent:"center"}}><ChevronLeft size={16}/></button>
            <button onClick={goNext} disabled={!hasNext} style={{width:32,height:32,borderRadius:8,border:`1px solid ${C.border}`,background:C.white,cursor:hasNext?"pointer":"not-allowed",opacity:hasNext?1:0.35,display:"flex",alignItems:"center",justifyContent:"center"}}><ChevronRight size={16}/></button>
          </div>
        </header>
        <div style={{flex:1}}>
          <MV mod={active} onComplete={()=>mark(activeId)} onNext={goNext} isCompleted={done(activeId)}/>
        </div>
        <footer style={{borderTop:`1px solid ${C.border}`,background:C.white,padding:"0.875rem 1.5rem",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          <button onClick={goPrev} disabled={!hasPrev} style={{display:"flex",alignItems:"center",gap:7,padding:"9px 16px",background:"transparent",border:`1px solid ${C.border}`,borderRadius:10,cursor:hasPrev?"pointer":"not-allowed",opacity:hasPrev?1:0.35,fontSize:13,color:C.ink,fontFamily:sans}}>
          ...
          <button onClick={goNext} disabled={!hasNext} style={{display:"flex",alignItems:"center",gap:7,padding:"9px 16px",background:hasNext?C.ink:C.border,color:hasNext?C.white:C.muted,border:"none",borderRadius:10,cursor:hasNext?"pointer":"not-allowed",fontSize:13,fontWeight:500,fontFamily:sans}}>
